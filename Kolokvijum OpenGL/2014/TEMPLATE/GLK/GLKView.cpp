...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_UP) m_glRenderer.alfa += 10;
	if (nChar == VK_DOWN) m_glRenderer.alfa -= 10;
	if (nChar == VK_LEFT) m_glRenderer.beta += 10;
	if (nChar == VK_RIGHT) m_glRenderer.beta -= 10;
	if( nChar == 'Q') m_glRenderer.pomeraj[0] -= 10;
	if( nChar == 'W') m_glRenderer.pomeraj[0] += 10;
	if( nChar == 'A') m_glRenderer.pomeraj[1] -= 10;
	if( nChar == 'S') m_glRenderer.pomeraj[1] += 10;
	if (nChar == 'Z') m_glRenderer.pomeraj[2] -= 10;
	if (nChar == 'X') m_glRenderer.pomeraj[2] += 10;
	Invalidate();
